...
# 1. 目的 (Objective)
本レポートの目的は,信号処理論において学んだ，フーリエ変換・窓関数を線形代数の視点から捉え直すことである．
...
オイラーの公式を用いて，この定義式を指数関数に分解する．また，簡単のため $N-1 \approx N$ と近似しよう．
...
$$
w[n]=0.5\cdot e^{i\frac{2\pi \cdot 0 \cdot n}{N}}-0.25\cdot e^{i\frac{2\pi \cdot 1\cdot n}{N}} - 0.25\cdot e^{-i\frac{2\pi \cdot 1\cdot n}{N}}
$$

- 第1項: 定数項．周波数 $k=0$ の成分．
- 第2項: 正の回転成分．周波数 $k=1$ の基底ベクトルと見なせる．
- 第3項: 負の回転成分．周波数 $k=-1$ （DFTの周期性により $k=N-1$ ）の成分．

以上より，ハニング窓を周波数領域でみると，上記の周波数以外ではゼロを示すため，3本のインパルスでしかない．

$$
W_{Hann}=[-0.25, 0.5, -0.25]
$$

信号 $x[n]$ にハニング窓 $w[n]$ を掛ける．

$$
y[n]=x[n]\cdot w[n]
$$

フーリエ変換の畳み込み定理によれば，これは「周波数の畳み込み」になる．

$$
Y[k]=X[k]*W[k]
$$

$X[k]$ に $W_{Hann}=W[k]$ を畳み込むとは以下を意味する．

$$
X_{new}[k]=0.5X[k]-0.25(X[k-1]+X[k+1])
$$

このため，ハニング窓を適用すると，「ピークは少し低くなり，裾野（サイドローブ）が互いに打ち消し合って消滅する」．また，ハニング窓の影響はピークだけでなく，その周辺にも及ぶ．このため，ピーク周辺もやや強調され，突出したピークは見えなくなる，すなわち山の幅が広くなる（周波数分解能の低下）．

これにより，振幅分解能は多少犠牲になるが，スペクトル漏れによる信号の汚染を防ぐことができる．プログラムでは「ピークが低くなる」という影響を補整するために，定数倍している．
...
なぜこれらの現象が起きたのか．3つの観点から説明される．

- 入力信号: 周波数2.5Hz．これはDFTのビン $k=2$ と $k=3$ のちょうど中間である．
- 矩形窓の特性: 周波数応答はsinc関数（ $\sin (x) / x$）である．sinc 関数は中心から0.5ずれると，値はピークの約0.64倍まで急落する．
- ハニング窓の特性: 周波数応答は，3つのsinc 関数の和である．メインローブ（山の幅）が矩形窓の2倍の太さを持つため，中心から0.5ずれても頂上付近の平らな部分に留まり，値は約0.85倍程度の低下で済む．

故に，コードで行った定数倍の補整に関わらず，純粋な窓関数の特性だけで，ピークのエネルギーの大小は説明される．窓関数の特性とは，ビンの隙間への感度と言えるだろう．
